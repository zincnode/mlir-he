...
namespace {
// This pattern converts an arith::AddIOp to a mlir_he::he::AddOp.
struct ConvertArithAddIOp : public mlir::OpRewritePattern<mlir::arith::AddIOp> {
  ...
  mlir::LogicalResult matchAndRewrite(mlir::arith::AddIOp op, mlir::PatternRewriter &rewriter) const override {
    rewriter.replaceOpWithNewOp<mlir_he::he::AddOp>(op, op.getType(), op.getLhs(), op.getRhs());
    return mlir::success();
  }
};

// This pattern converts an arith::AddFOp to a mlir_he::he::AddOp
struct ConvertArithAddFOp : public mlir::OpRewritePattern<mlir::arith::AddFOp> {
  ...
  mlir::LogicalResult matchAndRewrite(mlir::arith::AddFOp op, mlir::PatternRewriter &rewriter) const override {
    rewriter.replaceOpWithNewOp<mlir_he::he::AddOp>(op, op.getType(), op.getLhs(), op.getRhs());
    return mlir::success();
  }
};
...
std::unique_ptr<mlir::Pass> mlir_he::createConvertArithToHePass() { return std::make_unique<ArithToHe>(); }
